                    ...
                    <div class="table-box">
                        ...
                        <div class="col-md-4" >
                            <div class="form-group form-inline">
                                <div class="btn-group">
                                    <button type="button"  @click="confirmReserves"class="btn btn-default"
                                            title="确认借书"><i class="fa fa-check"></i> 确认借书</button>
                                    <button type="button" @click="getReserveBooks('')" class="btn btn-default"
                        ...
                        <form id="searchForm">
                            <div class="col-md-6 form-group" style="display: flex;" v-cloak>
                                ...
                                <div class="col-md-2 data" style="display: flex;">
                                    <input type="number" name="rbbId" autocomplete="off" required class="form-control" @keydown="verifyNum($event)" placeholder="编号" >
                                </div>
                        ...
                        <table id="dataList" class="table table-bordered table-striped table-hover dataTable">
                            ...
                            <tr>
                                ...
                                <th class="text-center">借阅天数</th>
                                <th class="text-center">订单时间</th>
                                <th class="text-center">操作</th>
                            </tr>
                            </thead>
                            <tbody>
                            <tr v-if="showLoading"><td colspan="8" style="text-align: center;"><img style="width: 5%;" src="<%=basePath%>img/loading.gif"></td></tr>
                            <tr v-cloak v-if="showData" v-for="(borrowBook,index) in reserveBorrowBooks" style="text-align: center;" :key="index">
                                <td><input class="icheckbox_square-blue" v-bind:id="borrowBook.rbbId" name="ids" type="checkbox" v-bind:value="borrowBook.rbbId"></td>
                                <td>{{borrowBook.rbbId}}</td>
                                <td>{{borrowBook.bname}}</td>
                                <td>{{borrowBook.rname}}</td>
                                <td>{{borrowBook.remainingDays}}</td>
                                <td>{{borrowBook.orderTime}}</td>
                                <td class="text-center">
                                    <button type="button" class="btn bg-olive btn-xs"
                                            @click="confirmReserve(index,borrowBook.rbbId)" data-target="#modifyBook">确认借书</button>
                                </td>
                            </tr>
                            <tr v-if="showMessage"  v-cloak style="text-align: center;font-size: 1.5em">
                                <td colspan="8" style="letter-spacing: 5px;padding: 15px;">{{message}}</td>
                            </tr>
                            </tbody>
                        </table>
                        <!--数据列表/-->
    ...
    let app = new Vue({
        ...
        methods: {
            ...
            searchBooks:async function(){
                ...
                if(searchObj.rbbId != undefined && searchObj.rbbId != ""){
                    searchUrl += "&rbbId="+searchObj.rbbId;
                }
                searchUrl = searchUrl.substr(1);
                this.currentPage = 1;
                await this.getReserveBooks(searchUrl);
                this.pageNumReload();
            },
            //确定借书 一个
            confirmReserve:function(index,id){
                axios.post("<%=basePath%>manager/reserveBorrow/"+id)
                    .then(response => {
                        console.log(response);
                        this.reserveBorrowBooks.splice(index,1); //页面中图书
                        if(this.reserveBorrowBooks.length == 0){
            ...
            confirmReserves:function(){
                ...
                if (confirm("您确定借出书选中的图书吗?")) {
                    axios.post("<%=basePath%>manager/reserveBorrow",ids)
                        .then(response => {
                            console.log(response);
                            let reserveBorrowBooks = this.reserveBorrowBooks;
                            for(let i=0;i<ids.length;i++){
                                for(let j=0;j<reserveBorrowBooks.length;j++){
                                    console.log(reserveBorrowBooks[j].rbbId,ids[i],typeof reserveBorrowBooks[j].rbbId,typeof ids[i]);
                                    if(reserveBorrowBooks[j].rbbId == ids[i]){
                                        console.log(ids[i]);
                                        reserveBorrowBooks.splice(j,1);
                                    }
                                }
                            }
                            this.$set(this.reserveBorrowBooks,reserveBorrowBooks);
                            alert("借书成功");
                            if(this.reserveBorrowBooks.length == 0){
                                this.getBooksAndReloadPageNum();
                            }
            ...
            //分页获取要借书的读者
            getReserveBooks: async function (searchUrl = "") {
                let url = "<%=basePath%>manager/reserveBorrow/"+this.currentPage+"/"+this.size;
                ...
                await axios.get(url).then(response => {
                    console.log(response)
                    this.reserveBorrowBooks = response.data.rows;
