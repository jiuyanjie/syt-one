                    ...
                    <div class="col-md-12">
                        ...
                        <div class="box box-primary">
                            <div class="date box-header with-border">
                                <div class="col-md-9">
                                    <h3 class="box-title">借书/归还 统计</h3>
    ...
    <script>
        ...
        let vue = new Vue({
                    ...
                    toolbox: {
                        ...
                        feature: {
                            ...
                            myTool1: {
                                show: true,
                                title: '借书人数降序',
                                icon: 'image://<%=basePath%>img/sort.png',
                                onclick:()=>{
                                    //降序
                                    vueT.GBObj.sort(function(a,b){
                                        return b.borrowNumber - a.borrowNumber;
                                    });
                                    console.log(vueT.optionLine.series[0].type);
                                    vueT.sortSub();
                                },
                            },
                            myTool2: {
                                ...
                                onclick:()=>{
                                    //降序
                                    vueT.GBObj.sort(function(a,b){
                                        return b.giveBackNumber - a.giveBackNumber;
                                    });
                                    vueT.sortSub();
                                },
                            },
                            myTool3: {
                                show: true,
                                title: '还原排序',
                                icon: 'image://<%=basePath%>img/restor.png',
                                onclick:()=>{
                                    //复制
                                    let option = vueT.optionLine;
                                    option.series[0].data = vueT.beforeBB;
                                    option.series[1].data = vueT.beforeGB;
                                    option.xAxis[0].data = vueT.beforeD;
                                    vueT.myChartLine.setOption(option);
                                },
                            },
            ...
            methods: {
                ...
                //从服务器获取折线图数据
                getLineData: function (id, dateStr) {
                    console.log(dateStr);
                    ...
                    console.log(index,start,end);
                     axios.post("<%=basePath%>manager/GBStatistics/"+id+"/"+start+"/"+end).then( response => {
                        console.log(response);
                        let lineData = response.data;
                        let option = this.optionLine;
                        option.xAxis[0].data = this.beforeD = lineData.days; //日期 （*号）
                        option.series[0].data = this.beforeBB = lineData.borrowNumbers;  //借书人数
                        option.series[1].data = this.beforeGB = lineData.giveBackNumbers; //还书人数
                        // 使用刚指定的配置项和数据显示图表。
                        this.optionLine = option;
                        this.myChartLine.setOption(option);
                        //组合称对象
                        let GBObj = this.GBObj;
                        for (let i = 0; i < lineData.days.length; i++) {
                            GBObj.push({
                                "borrowNumber": lineData.borrowNumbers[i],
                                "giveBackNumber": lineData.giveBackNumbers[i],
                                "day": lineData.days[i]
                            });
                        }
                ...
                sortSub(){
                    ...
                    let date2 = [];
                    let days = [];
                    let GBObj = vueT.GBObj;
                    for(let i=0;i<GBObj.length;i++){
                        let obj = GBObj[i];
                        date1.push(obj.borrowNumber);
                        date2.push(obj.giveBackNumber);
                        days.push(obj.day);
                    }
                    //赋值
                    let option = vueT.optionLine;
                    option.series[0].data = date1;
                    option.series[1].data = date2;
