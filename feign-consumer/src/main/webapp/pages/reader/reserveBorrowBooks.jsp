                    ...
                    <div class="table-box">
                            ...
                            <div class="form-group form-inline">
                                <div class="btn-group">
                                    <button type="button" class="btn btn-default" @click="deleteBooks"
                                            title="取消预定"><i class="fa fa-check"></i> 取消预定
                                    </button>
                                    <button type="button" @click="getBooksAndReloadPageNum" class="btn btn-default"
                                            title="刷新"><i class="fa fa-refresh"></i> 刷新
                                    </button>
                                </div>
                            </div>
                        </div>
                        <form id="searchForm">
                            <div class="col-md-6 form-group" style="display: flex;" v-cloak>
                                ...
                                <button type="reset" style="display: inline-block;" title="搜索" class="btn btn-default">
                                    重置
                                </button>
                            </div>
                        </form>
                        ...
                        <table id="dataList" class="table table-bordered table-striped table-hover dataTable">
                            ...
                            <tr v-if="showLoading">
                                <td colspan="8" style="text-align: center;">
                                    <img style="width: 5%;" src="<%=basePath%>img/loading.gif">
                                </td>
                            </tr>
                            <tr v-cloak v-if="showData" v-for="(borrowBook,index) in books" style="text-align: center;"
                                :key="index">
                                <td><input class="icheckbox_square-blue" v-bind:id="borrowBook.rbbId" name="ids"
                                           type="checkbox" v-bind:value="borrowBook.rbbId"></td>
                                ...
                                <td class="text-center">
                                    <button type="button" class="btn bg-orange btn-xs" data-toggle="modal"
                                            @click="modifyBookIndex = index,remainingDaysModify=borrowBook.remainingDays"
                                            data-target="#modifyBook">修改天数
                                    </button>
                                    <button type="button" class="btn bg-red btn-xs"
                                            @click="deleteBook(index)">取消预定
                                    </button>
                                </td>
                            </tr>
...
<%--                            </select>--%>
                    ...
                    <div class="box-tools pull-right">
                        <ul class="pagination">
                            ...
                            <li>
                                <a v-cloak v-if="page > pageNumArr[pageNumArr.length-1]">...</a>
                                <!--总页数 大于 当前显示的最后一个元素的页码-->
                            </li>
                        </ul>
                    </div>
                </div>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content" v-cloak>
                    <div class="modal-header">
                        ...
                        <h3 class="modal-title" id="myModalLabel" style="text-align: center;">修改天数</h3>
                    </div>
                        ...
                        <div class="tab-pane" id="tab-common">
                            <form id="modifyBookForm">
                                <div class="row data-type">
                                    ...
                                    <div class="col-md-4 title">借阅天数</div>
                                    ...
                                    <div class="col-md-12 data text-center" style="margin-top: 20px;">
                                        <button type="button" @click="modifyBookByForm"
                                                class="btn bg-maroon">确认修改
                                        </button>
                                        <button type="button" class="btn bg-default" data-dismiss="modal">
                                            返回
                                        </button>
                                    </div>
                                </div>
    ...
    let app = new Vue({
        ...
        methods: {
            ...
            deleteBooks:function(){
                ...
                if (confirm("您确定取消预定选中的图书吗?")) {
                    axios.delete("<%=basePath%>reader/reserveBorrow", {data: ids}).then(response => {
            ...
            deleteBook:function(index){
                if(confirm("您确认取消预定吗？")){
                    axios.delete("<%=basePath%>reader/reserveBorrow/"+this.books[index].rbbId)
                        .then(response => {
                            this.getBooksAndReloadPageNum();
                            alert("取消成功！");
                        }).catch(err => {
            ...
            modifyBookByForm: function () {
                if(this.remainingDaysModify == this.books[this.modifyBookIndex].remainingDays){
                    console.log(false);
                    return false;
                }
                if(this.remainingDaysModify == '' || Number(this.remainingDaysModify) == NaN || Number(this.remainingDaysModify)<=0){
                    alert('请输入有效天数！');
                    return false;
                }
                axios.put("<%=basePath%>reader/reserveBorrow/"+this.books[this.modifyBookIndex].rbbId+ "/" + this.remainingDaysModify)
                    .then(response => {
                        console.log(response);
                        this.books[this.modifyBookIndex].remainingDays = this.remainingDaysModify;
            ...
            getReserveBooks: async function (searchUrl = "") {
                let url = "<%=basePath%>reader/reserveBorrow/" + this.currentPage + "/" + this.size;
